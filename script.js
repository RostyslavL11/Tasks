...
{
    // 191 В переменной arr содержится некоторый массив с числами. Напишите условие, которое проверит, что в массиве 3 элемента. Если это так, выведите на экран сумму элементов массива.
    let arr = [1, 2, 5];
    let sum = 0;
    if (arr.length == 3) {
        // 1 way
        ...
        // 3 way 
        sum = arr.reduce((total, amount) => {
            return total + amount;
        });
        // console.log(sum);

    } else {
        // console.log('wrong quantity of numbers');
    }
}

{
    // 192 Дана переменная, содержащая некоторую строку. Проверьте, что эта строка начинается на символ 'a'.
    let str = 'abcd';
    if (str[0] == 'a') {
        // console.log('a');
    }
...
{
    // 193 Дана переменная, содержащая некоторую строку. Проверьте, что эта строка заканчивается на символ 'x'.
    let str = 'abcdx';
...
{
    // 194 Дана переменная, содержащая некоторую строку. Проверьте, что эта строка начинается на символ 'a' или символ 'b'.
    let str = 'cbcd';
    if (str[0] == 'a' || str[0] == 'b') {
        // console.log('a or b');
    } else {
...
{
    // 195 Дано целое число. Напишите условие, которое проверит, равна ли последняя цифра этого числа нулю.
    let numbers = 1234567890;
    let last = String(numbers);
...
{
    // 196 Пусть в переменной num хранится число. Определите, четное число или нет. Число будет четным, если последний символ равен 0, 2, 4, 6 или 8, и нечетным в противном случае.
    let number = 0;
...
{
    // 197 Дано число. Проверьте, что оно делится на 3.
    let number = 10;
    let rest = number % 3;
    if (rest == 0) {
        // console.log(`число ділиться на 3 без залишку`);
    } else {
        // console.log(`залишок від ділення на число 3, буде: ${rest}`);
    }
}

{
    // 198 Код должен проверить равна ли сумма первых трех цифр числа сумме вторых трех чисел:
    let num = '123006'; // берем в кавычки, чтобы обращаться к цифрам
...
{
    // 199 Дано число, например, 12345. Проверьте, что первым символом этого числа является цифра 1, 2 или 3. Если это так - выведите 'да', в противном случае выведите 'нет'.
...
{
    // 200 Дано трехзначное число. Найдите сумму цифр этого числа.
    let number = 550;
    let str = String(number);
    let sum = 0;
    for (let i = 0; i < str.length; i++) {
...
{
    // 201 Дано число из 6-ти цифр. Проверьте, что сумма первых трех цифр равняется сумме вторых трех цифр. Если это так - выведите 'да', в противном случае выведите 'нет'.
    let num = 104005;
    let str = String(num);
...
{
    // 202 обертаємо попередню задачу в функцію 
    let num = 104005;
    const compareNumbers = function (number) {
        let str = String(number);
        let sum1 = 0;
        let sum2 = 0;
...
{
    // 203 Написать тернарный оператор, который будет выводить в консоль сообщение о том, является ли заданное число четным или нечетным. Например, для числа 7 выводится сообщение "нечетное число", а для числа 10 - "четное число".
    let num = 4;
    // num % 2 == 0 ? console.log('парне число') : console.log('не парне число');
}

{
    // 204 Написать тернарный оператор, который будет проверять, принадлежит ли заданное число диапазону от 10 до 100. Если число входит в этот диапазон, то выводится сообщение "число входит в диапазон", в противном случае - "число не входит в диапазон".
    let num = 15;
    // num >= 10 || num <= 100 ? console.log('число входит в диапазон') : console.log('число не входит в диапазон');
}
...
{
    // 206 Написать тернарный оператор, который будет проверять, содержит ли заданный объект свойство "name". Если свойство есть, то выводится сообщение "объект содержит свойство name", в противном случае - "объект не содержит свойство name".
    let obj = {
        age: 25,
        name: "John"
    };
